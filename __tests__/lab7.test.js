describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const btnTextHandle = await button.getProperty('innerText');
    const btnText = await btnTextHandle.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    await page.reload();
    await page.waitForSelector('product-item:nth-child(20)');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    await page.waitForSelector('product-item:nth-child(20)',  { timeout: 10000 });
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      if (btnText !== 'Remove from Cart') {
        console.log(`Item ${i + 1} button says: ${btnText}`);
